...
import fr.utbm.ciad.labmanager.views.components.charts.barchart.BarCharPublicationTest;
...
public class ChartsView extends HorizontalLayout {
    ...
    public ChartsView(@Autowired PublicationService publicationService) {
        ...
        //BarCharPublicationTest barChartPublication = new BarCharPublicationTest(this.publicationService);
        //List<BarChart> barChartList = barChartPublication.getBarChartList();
        LineChartPublication lineChartPublication = new LineChartPublication(this.publicationService);
        ...
        // Define axes
        YAxis yAxis = new YAxis(DataType.NUMBER); // Just need the value type as parameter
        XAxis xAxis = new XAxis(xValues);
        xAxis.setMin(xValues.getMin());
        xAxis.setMax(xValues.getMax());// We want to start the X axis from minimum of our data
        //xAxis.getLabel(true).setFormatter("{MMM}"); // Format the date
        xAxis.setName("Years");

        RectangularCoordinate rc = new RectangularCoordinate(xAxis, yAxis);
        /*
        for(int i = 0; i < barChartPublication.getTypeSize(); i++){
            barChartList.get(i).setStackName("S");
            barChartList.get(i).plotOn(rc);
        }
         */
        ...
        lineChartPublication.plotOn(rc);
        ...
        legend.getPosition(true).setLeft(Size.percentage(10)); // Leave 10% space on the right
